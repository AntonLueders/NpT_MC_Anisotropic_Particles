...
int MetropolisTranslation(int index, Particle *P, double L) {
    ...
    while (current != NULL) {
		...
		if (i != index) {
			...
			double disji = L;
			if (disij < 3. / 2. * sigma) {
				disji = Distance(&(P[i]), &test_particle, L);     // See distance.c
			}
...
int MetropolisRotation(int index, Particle *P, double L) {
    ...
    while (current != NULL) {
		...
		if (i != index) {
			...
			double disji = L;
			if (disij < 3. / 2. * sigma) {
				disji = Distance(&(P[i]), &test_particle, L); 
			}
			if(disij < sigma || disji < sigma) {
		...
		if (L_temp <= L) {
			...
			while (current != NULL) {
				...
				if (j < i) {
					...
					double disji = L;
					if (disij < 3. / 2. * sigma) {				
						disji = Distance(&(test_particles[j]), &(test_particles[i]), L_temp);     // See distance.c
					}
					if(disij < sigma || disji < sigma) {
						return L;
					}
